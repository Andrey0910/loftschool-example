        ...
        if (nodes.length > 0) {
            Array.from(nodes).forEach(value => {
                if (value.nodeType === 1) {
                    ...
                    obj.tags.hasOwnProperty(valueTag) ? obj.tags[valueTag]++ : obj.tags[valueTag] = 1;

                    if (valueClass.length) {
                        valueClass.forEach(item => {
                            obj.classes.hasOwnProperty(item) ? obj.classes[item]++ : obj.classes[item] = 1;
                        });
                    }
    ...
    let observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            ...
            if (Object.keys(mutation.addedNodes).length) {
                ...
                for (let value of mutation.addedNodes) {
